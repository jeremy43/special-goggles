...
import mxnet as mx
from core.tester import im_detect, Predictor,vis_all_detection,get_resnet_output,prepare_data,draw_all_detection
from symbols import *
...
def process_pred_result(classes,pred_result, num_classes, thresh, cfg, nms, all_boxes, idx, max_per_image, vis, center_image, scales):
    for delta, (scores, boxes, data_dict) in enumerate(pred_result):
        ...
        if cfg.TEST.SEQ_NMS==False and  max_per_image > 0:
            ...
            # if vis:
            #     vis_all_detection(center_image, boxes_this_image, classes, scales[delta], cfg)

            #delta always be 0 here
            return out_im
    return 0
...
def main():
    ...
    image_names=image_names[0:15]
    data = []

    all_boxes = [[[] for _ in range(len(image_names))]
    ...
    print 'time of data',time.time()-t1
    t1=time.time()
    provide_data = [[(k, v.shape) for k, v in zip(data_names, data[i])] for i in xrange(len(data))]
    print 'time of provide_data',time.time()-t1
    ...
    while start_counter < cfg.TEST.KEY_FRAME_INTERVAL + 1:
    ...
    vis = False
    t1 = time.time()
    print 'len of image_names',len(image_names)
    for idx, im_name in enumerate(image_names[1:]):
        ...
        if(idx != len(image_names)-2):
            ...
            if len(data_list) < all_frame_interval - 1:
                ...
                out_im=process_pred_result(classes,pred_result, num_classes, 1e-3, cfg, nms, all_boxes, idx, max_per_image,vis,
                                    data_list[cfg.TEST.KEY_FRAME_INTERVAL].asnumpy(), scales)
                total_time=time.time()-t1
                file_idx += 1
                if (cfg.TEST.SEQ_NMS==False):
                    save_image(output_dir, count, out_im)
            ...
            while end_counter < cfg.TEST.KEY_FRAME_INTERVAL + 1:
                ...
                out_im=process_pred_result(classes,pred_result, num_classes, 1e-4, cfg, nms, all_boxes, idx, max_per_image, vis,
                                    data_list[cfg.TEST.KEY_FRAME_INTERVAL].asnumpy(), scales)
                ...
                if (cfg.TEST.SEQ_NMS == False):
                    save_image(output_dir, count, out_im)
                print 'testing {} {:.4f}s'.format(str(count)+'.JPEG', total_time / count)
                end_counter+=1
    ...
    if(cfg.TEST.SEQ_NMS):
        ...
        for idx in len(image_names):
            boxes_this_image = [[]] + [all_boxes[j][idx] for j in range(1, num_classes)]
            out_im = draw_all_detection(data[0][data], boxes_this_image, classes, scales[0], cfg)
            save_image(output_dir, idx+1, out_im)
