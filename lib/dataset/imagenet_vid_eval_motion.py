...
def vid_eval_motion(detpath, annopath, imageset_file, classname_map, annocache, motion_iou_file, motion_ranges, area_ranges, ovthresh=0.5):
   ...
   # load annotations from cache
    ...
    motion_ap = np.zeros((len(motion_ranges), len(classname_map) - 1))
    area_ap = np.zeros((len(area_ranges), len(classname_map) - 1))

    npos_bak = copy.deepcopy(npos)
    motion_npos = copy.deepcopy(npos)
    area_npos = []

    area_tp_cell = []
    area_fp_cell = []

    for area_range_id in range(len(area_ranges)):
        area_tp_cell.append([None] * num_imgs)
        area_fp_cell.append([None] * num_imgs)
        area_npos.append(copy.deepcopy(npos))

    for motion_range_id, motion_range in enumerate(motion_ranges):
        motion_tp_cell = [None] * num_imgs
        motion_fp_cell = [None] * num_imgs
        for area_range_id, area_range in enumerate(area_ranges):
            ...
            for index, rec in enumerate(recs):
                ...
                motion_tp_cell[id] = np.logical_or(motion_tp_cell[id], tp)
                motion_fp_cell[id] = np.logical_or(motion_fp_cell[id], fp)

                area_tp_cell[area_range_id][id] = np.logical_or(area_tp_cell[area_range_id][id], tp)
                area_fp_cell[area_range_id][id] = np.logical_or(area_fp_cell[area_range_id][id], fp)
                ...
                for k in range(0, num_gt_obj):
                    ...
                    # avoid repeat deducting
                    if area_range_id == 0:
                        if ig_gt_motion[k]:
                            motion_npos[label] = motion_npos[label] - 1
                    if motion_range_id == 0:
                        if ig_gt_area[k] :
                            area_npos[area_range_id][label] = area_npos[area_range_id][label] - 1
        ...
        motion_ap[motion_range_id] = calculate_ap(motion_tp_cell, motion_fp_cell, gt_img_ids, obj_labels_cell, obj_confs_cell, classname_map, motion_npos)
        motion_npos = copy.deepcopy(npos_bak)

    return ap, motion_ap, area_ap
